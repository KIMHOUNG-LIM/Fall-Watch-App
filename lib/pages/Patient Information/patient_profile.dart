...
class _PatientProfilePageState extends State<PatientProfilePage> {
  ...
  List<Map<String, dynamic>> fallDetections = [];

  @override
    void initState() {
      super.initState();
      _fetchFallHistoryOfPatient();
    }

    Future<void> _fetchFallHistoryOfPatient() async{
      try{
        final QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('Fall Detections')
                            .where('cameraId', isEqualTo: widget.cameraId)    
                            .where('room', isEqualTo: widget.location)
                            .get();
        if (querySnapshot.docs.isNotEmpty){
          setState((){
            fallDetections = querySnapshot.docs
                            .map((doc) => doc.data() as Map<String, dynamic>)
                            .toList();
          });
        }else{
          print(" No Fall History Found For this Patient");
        }
              
      }catch(e){print("Error fetching fall history of Patient");}
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Fetch Fall Detection by room and cameraId
    

    // Filter fall detections based on location and camera ID
    final filteredFallDetections = fallDetections.where((fall) {
      // Trim any whitespace
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Patient's Profile", style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
      ),
      body: Padding(
        ...
        ElevatedButton(
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => RelativeContactPage(patientDocumentId: widget.patientDocId,),
              ),
            );
          },
          child: Column(
